...
const Create: NextPage = () => {
  ...
  const [fundDeployed, setFundDeployed] = useState(0)
            ...
            <Flex direction={"column"} gap={"20px"}>
                  ...
                  <Flex direction={"row"} justifyContent={"space-evenly"} alignItems={"center"}>
                    ...
                    <Flex whiteSpace={"nowrap"} direction={"row"} gap={"10px"}>
                      <Text fontWeight={"bold"} fontSize={"4xl"}>
                        1 {props.values.symbol} = </Text><Text color={"#f6643c"} fontWeight={"bold"} fontSize={"4xl"}>{(props.values.amount / props.values.shareAmount).toPrecision(3)}</Text><Text fontWeight={"bold"} fontSize={"4xl"}>{Asset[denominationAsset].id.toString()}{" "}</Text></Flex>
              ...
              <Modal isOpen={isOpen} onClose={() => setIsOpen(false)}>
                ...
                <ModalContent backgroundColor={"#0f0b1f"}>
                  {txAcceptedTransac == 0 ?
                    <ModalHeader color={"white"}>  Get Ready ⏳</ModalHeader>
                    :
                  ...
                  <ModalBody>
                    {txAcceptedTransac == 0 ?
                      <Flex direction={"column"}>
                        <Text fontWeight={"2x1"} color={"white"}>Your transaction is about to be confirmed on L2</Text>
                  ...
                  Fund on Goerli
                      ...
                      :
                      <Flex direction={"column"}>
                        <Text fontWeight={"2x1"} color={"white"}>Congrats, your fund has been Initalized, so Bullish 🙌</Text>
                        <Text color={"white"}>
                          Fund Address : {deployedVaultAddress}
                        </Text>
